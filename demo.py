...
import shutil
from  yolo3.calculate import outcome
...
def GetBoxesPic(image, boxes):
    ...
    tmp_img_dir = os.path.join(os.path.dirname(__file__), 'yolo3', 'tmp_img')
    # print(f"{tmp_img_dir=}")
    # 检查目录是否存在，如果存在则删除
    ...
    for i in range(len(boxes)):
        ...
        pic.save(tmp_img_dir+ '/pic' + str(i).rjust(3, '0') + '.jpg')
        pics.append(pic)
    return pics
...
def painting(equations, image, boxes):
    ...
    font = ImageFont.truetype('arial', image.size[1] // 50, encoding="utf-8")
    for i in range(len(boxes)):
        top, left, bottom, right = get4pos(boxes[i], image)
